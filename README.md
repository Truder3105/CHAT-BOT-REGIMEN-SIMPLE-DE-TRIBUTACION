...
**config.local.js** (tú lo creas; no versionar)



window.__RST_CONFIG__.GEMINI_API_KEY = "AIza…"; opcionalmente GEMINI_MODEL (solo modelos de texto, nunca *-tts).
...
Valores por defecto vacíos; se carga siempre antes de config.local.js en app.js.
...
Si montas Express/FastAPI como proxy, define GEMINI_API_KEY ahí. Ver plantilla **.env.example**.
...
Clona el repositorio o copia la carpeta rst-landing.



En VS Code: File → Open Folder → selecciona **rst-landing**.



Copia config.local.example.js → **config.local.js** y pega tu clave de Google AI Studio.



Abre index.html con Live Server (o npx serve rst-landing).



Usa el botón «Chat IA»; prueba ES / EN y preguntas sobre RST / facturación / filosofía.
